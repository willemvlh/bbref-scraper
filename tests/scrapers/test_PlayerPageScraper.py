...
def carmelo_anthony():
    return PlayerPageScraper(get_resource("carmelo_anthony.html")).get_content()
...
class TestPlayerPageScraper:
    ...
    def test_equality(self):
        ...
        lbj = lebron_james()
        assert (carmelo != lbj)
        _set = {carmelo, carmelo}
        assert len(_set) == 1

    def test_name(self):
        carmelo = carmelo_anthony()

        assert carmelo.name == "Carmelo Anthony"

    def test_get_dob(self):
        carmelo = carmelo_anthony()
        dob = carmelo.date_of_birth
    ...
    def test_get_id(self):
        ...
        assert carmelo.id == "carmelo_anthony"

    def test_get_shooting_hand(self):
        carmelo = carmelo_anthony()

        assert carmelo.shooting_hand == "Right"

    def test_get_college(self):
        carmelo = carmelo_anthony()
        lbj = lebron_james()
        assert carmelo.college == "Syracuse"
        assert lbj.college is None
    ...
    def test_game_log(self):
        ...
        assert len([game for game in gl if game.played]) == seasons[0].stats.games_played

    def test_all_star(self):
        erving = PlayerPageScraper(get_resource("julius_erving.html")).get_content()
        assert len([season for season in list(erving.seasons) if season.all_star]) == 11
    ...
    def test_salaries_none(self):
        assert wilt_chamberlain().salaries == []

    def test_draft_pick(self):
        dp = lebron_james().draft_pick
    ...
    def test_seasons_and_playoffs_are_generators(self):
        lbj = lebron_james()
        assert isinstance(lbj.seasons, Generator)
        assert isinstance(lbj.playoffs, Generator)
